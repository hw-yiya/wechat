    ...
    <div class="data-table table-striped">
        <table>
            ...
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php switch($channel["status"]): case "-1": ?>审核不通过<?php break;?>
                                <?php case "1": ?>审核通过<?php break;?>
                                <?php default: ?>待审核<?php endswitch;?>
                        </td>
                        <td>
                            <?php if($channel["status"] == 0): ?><a href="<?php echo U('setStatus?ids='.$channel['id'].'&status=0');?>" class="ajax-get">审核不通过</a><?php endif; ?>
                            <?php if($channel["status"] == 0 OR $channel["status"] == -1): ?><a href="<?php echo U('setStatus?ids='.$channel['id'].'&status=1');?>" class="ajax-get">审核通过</a><?php endif; ?>
